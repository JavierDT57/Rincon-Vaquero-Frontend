...
test.describe("Avisos - Crear aviso (correcto)", () => {
  ...
  test("Crear aviso correctamente como admin", async ({ page }) => {
    ...
    await page.getByTestId("aviso-imagen").setInputFiles("tests/assets/error.png"); //Imagen muy pesada

    // Enviar formulario
    await page.getByTestId("aviso-submit").click();

    // Verificar mensaje de error 
    await expect(page.getByTestId("aviso-error")).toBeVisible({ timeout: 8000 });
  });
});
